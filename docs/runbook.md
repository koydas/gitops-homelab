...
| `monitoring` Application shows `Healthy` but `OutOfSync` even right after a clean sync, or Grafana login stops working with the password from the runbook | Not just cosmetic: confirmed the Grafana subchart's auto-generated admin-password `Secret` value *does* change across some ArgoCD syncs (its `lookup()`-based reuse doesn't reliably hold), and since Grafana has no persistent storage (`grafana.persistence` not enabled), its SQLite user DB is rebuilt from whatever the `Secret` holds *at pod-boot time* — any pod restart between two syncs with different secret values leaves the running pod's actual login password out of sync with what `kubectl get secret` reports. Always re-fetch the password fresh (Common Tasks above) rather than reusing one from memory or an old chat. Root fix (not yet applied): enable `grafana.persistence` with a PVC so the admin user's password survives pod restarts independent of the Secret's value. |
