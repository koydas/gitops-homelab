...
In practice, that `lookup`-based reuse did not reliably hold across every ArgoCD sync — the generated password value changed repeatedly with no config change on our end. `grafana.persistence` (1Gi, `microk8s-hostpath`) was added first, but that alone wasn't sufficient — it stops the SQLite user DB from being wiped on restart, but doesn't stop the *Secret itself* from drifting, so a stale-looking `kubectl get secret` value could still be reported. Ultimately switched to `grafana.admin.existingSecret`, pointed at a Secret created once out-of-band (`grafana-admin-credentials`, not in Git — same "generated once, not templated" pattern as ArgoCD's own admin password). With `existingSecret` set, the chart never templates a password value at all, so there is nothing left for ArgoCD to drift. See [runbook.md](./runbook.md) for the exact incident writeup.
